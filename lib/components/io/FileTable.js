  ...
  componentWillReceiveProps(props) {
    if (props.files && (props.files.length !== this.props.files.length)) {
      const items = props.files.map(file => {
        return {
          thumb: `http://io-api.3nit.io/out/${DEFAULT_PROJECT}/${file.fileId}/img_fill/x400`,
          msrc: `http://io-api.3nit.io/out/${DEFAULT_PROJECT}/${file.fileId}/img_fill/x400`,
          src: `http://io-api.3nit.io/out/${DEFAULT_PROJECT}/${file.fileId}/img_fit/x800`,
          w: file.info.width,
          h: file.info.height,
          title: file.name,
          name: file.name,
    ...
    return (
          ...
          <Table striped>
            ...
            {this.state.items.map((file, index) => {
              ...
              return (
                <tr key={file._id}>
                  <td className="col-md-3 col-sm-4 col-xs-4">
                    <Image
                  ...
                  <td className="col-md-6 col-sm-4 col-xs-3">
                    <div style={{marginTop: 0, marginBottom: 0, }}>
                      ...
                      <h5>{file.name}</h5>
                      {file.info && <small>{file.info.width} x {file.info.height} ({file.info.type})</small>}<br />
