...
export function logIn(options) {
  ...
  const { email, password, history, redirect } = options
  return async dispatch => {
    try {
      ...
      dispatch(fetchProfile({token, history, redirect}))
    }
    catch (e) {
      debug('dev')(e)
...
export function register(options) {
  const {firstName, lastName, email, password, history} = options
  ...
  return async dispatch => {
    try {
      ...
      if (res.body._id) {
        ...
        storage.put('token', token)
        storage.put('email', email)
        dispatch(fetchProfile({token, history, redirect: '/profile'}))
...
export function reset(options) {
  ...
  return async () => {
    try {
      ...
      if (history) history.pushState(null, '/login')
    }
    catch (e) {
      debug('dev')(e)
    }
...
export function fetchProfile(options) {
  const { token, history, redirect } = options
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (history && redirect) history.pushState(null, validVAT ? redirect : '/profile')
    }
    catch (e) {
      debug('dev')(e)
      none()
